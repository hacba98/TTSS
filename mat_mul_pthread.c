...
int main(int argc, const char *argv[]){
    if(argc >= 3){
        ...
        for ( i = 0; i < num_threads; ++i ) {
            int rows = num_rows_per_thread[i]
            struct param_t param = {.id=i, .dim=dim, .m_a=matrix_a, .m_b=matrix_b, .m_r=result, .rows=num_rows_per_thread}
            pthread_create( &threads[i], NULL, worker, (void *) param );
        }
...
void mul_mat(int id, int *rows, int dim, double **matrix_a, double **matrix_b, double **result){
    ...
    int start = id * num_rows_per_thread[0];
    int end = start + num_rows_per_thread[id];
    //////////////////////////////////////////////////////////
    // TODO
