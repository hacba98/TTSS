...
int main(int argc, const char *argv[])
{
    if (argc >= 3)
    {
        ...
        int creation_flag, join_flag = 0;
        param_t *param_array = malloc(sizeof(param_t) * num_threads);
        for (i = 0; i < num_threads; i++)
        {
            int rows = num_rows_per_thread[i];
            param_array[i].id = i;
            param_array[i].dim = dim;
            param_array[i].m_a = matrix_a;
        ...
        for (i = 0; i < num_threads; i++)
        {
            join_flag = pthread_join(thread[i], NULL);
            if (join_flag)
            {
